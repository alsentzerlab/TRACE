...
from zeroshot_inference.extract_information.prompts  import INTIAL_SYS, INTIAL_USER, UPDATE_SYS, UPDATE_USER
...
# PROCESSING_TYPES = ['original', 'removed', 'removed_template', 'removed_copy', 'removed_stage1', 'removed_stage2'] # select the other modes

async def extract_information_call(row, text_column, iter, variables_df):
    """
...
async def extract_information(row, variables_df, semaphore):
    ...
    async def process_with_semaphore(row, text_column, iter):
        async with semaphore:
            return await extract_information_call(row, text_column, iter, variables_df)
